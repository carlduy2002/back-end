...
namespace web_project_BE.Controllers
{
    ...
    public class CartDetailController : ControllerBase
    {
            ...
            var ds = _context.Cart_details
                ...
                .Join(_context.Sizes,
                      ...
                      (joinResult, size) => new
                      {
                          ...
                          isCheck = joinResult.cartDetail.isChecked,
        ...
        [HttpPut("setIsCheck")]
        public async Task<IActionResult> setIsCheck()
        {
            var id = Request.Form["id"];

            if(int.Parse(id).Equals(0))
            {
                var c = _context.Cart_details.FirstOrDefault();
                var cd = _context.Cart_details.ToList();

                //foreach(var c in cd) m
                //{
                if (c.isChecked.Equals("false"))
                {
                        
                    foreach(var cds in cd)
                    {
                        cds.isChecked = "true";

                        _context.Entry(cds).State = EntityState.Modified;
            ...
            {
                var cd = _context.Cart_details.Where(cd => cd.cd_id.Equals(int.Parse(id))).FirstOrDefault();

                if (cd.isChecked.Equals("false"))
                {
                    cd.isChecked = "true";
