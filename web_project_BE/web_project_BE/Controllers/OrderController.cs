...
namespace web_project_BE.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> ProcessOrder([FromBody] CartAndOrder model)
        {
            ...
            var account_status = _context.Accounts
                .Where(a => a.account_id.Equals(account_id))
                .Select(a => a.account_status.ToString())
                .FirstOrDefault();

            if (account_status.Equals("Lock"))
            ...
            foreach (var c in cart)
            {
                ...
                .Where(cd => cd.cd_id == c.cd_id)
                .FirstOrDefault();

                _context.Cart_details.Remove(cartDateil);
                await _context.SaveChangesAsync();
            }
        ...
        public async Task<IActionResult> search(string key)
        {
            try
            {
                DateTime date = DateTime.ParseExact(key, "MMMM", CultureInfo.InvariantCulture);
                ...
                var lstorders = _context.Orders
                    ...
                    .Select(o => new
                    {
                        ...
                        order_total_price = o.order_total_price,
                        account_name = o.account.account_username
                    })
                    .Where(o => o.order_date.Month.Equals(monthNumber))
                    .ToList();
            ...
            catch (FormatException)
            {
                if (int.TryParse(key, out int result))
                {
                    var lstorders = _context.Orders
                          ...
                          .Where(o => o.order_date.Month.Equals(result))
                          .ToList();
                ...
                var orders = _context.Orders
                    ...
                    .Where(o => o.order_status.Contains(key))
                    .ToList();
